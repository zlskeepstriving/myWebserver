...
// 网站的根目录
const char* doc_root = "/home/zhaols/httpResources";
...
void http_conn::init(int conn_fd, const sockaddr_in& client_addr) {
    m_sockfd = conn_fd;
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text) {
    ...
    if (text[0] == '\0') {
        ...
        text += 11;
        text += strspn(text, " \t");
        if (strcasecmp(text, "keep-alice") == 0) {
            m_linger = true;
        }
